...
pub fn extract_mysql_create_table_column_definitions(
    ...
    for raw_line in create_table_sql.lines() {
        ...
        if is_mysql_generated_column_definition(def.as_str()) {
            continue;
        }
...
pub fn extract_mysql_generated_column_names(create_table_sql: &str) -> Vec<String> {
    ...
    for raw_line in create_table_sql.lines() {
        ...
        if is_mysql_generated_column_definition(line.trim_end_matches(',')) {
            result.push(line[1..second_tick].to_string());
        }
    }
    result
}

fn is_mysql_generated_column_definition(definition: &str) -> bool {
    let def_lower = definition.to_ascii_lowercase();
    def_lower.contains("stored generated")
        || def_lower.contains("virtual generated")
        || def_lower.contains("generated always")
}
...
mod tests {
  ...
  PRIMARY KEY (`id`)
        ...
        assert_eq!(
            extract_mysql_generated_column_names(sql),
            vec!["g".to_string()]
        );
    }
