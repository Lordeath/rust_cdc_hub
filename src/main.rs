...
const translations = {
  zh: {
    ...
    sync_stored_procedure: '同步存储过程/函数',
    overwrite_stored_procedure: '覆盖存储过程/函数',
    status_running: '运行中',
  ...
  en: {
    ...
    sync_stored_procedure: 'Sync Stored Routines',
    overwrite_stored_procedure: 'Overwrite Stored Routines',
...
function renderStatusData(data) {
    ...
    el('configKv').innerHTML = [
      ...
      kvRow(t('sync_stored_procedure'), cfg.sync_stored_procedure),
      kvRow(t('overwrite_stored_procedure'), cfg.overwrite_stored_procedure),
