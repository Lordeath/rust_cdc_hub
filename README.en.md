...
| `checkpoint_flush_interval_secs` | No | Flush interval for the checkpoint background task. Defaults to `30` seconds; `0` or omitted uses the default. |
...
Checkpoints are persisted with SQLite + WAL. On the first SQLite checkpoint startup, legacy `checkpoint_*.json` files in the same directory are imported automatically and kept as rollback backups. At runtime, only the `CheckpointService` background task owns the SQLite connection and performs writes. Source and sink components submit safe table-level checkpoints and binlog stream checkpoints into the in-memory queue; the service coalesces dirty state and flushes it according to `checkpoint_flush_interval_secs`. No-dirty cycles do not write to disk. When no target-table rows arrive for a long time but the binlog keeps advancing, the stream checkpoint records the latest safely consumed position, so restarts do not need to rescan a large old binlog range from the table checkpoint. SQLite WAL mode creates `-wal` and `-shm` files next to the checkpoint database. The status page and `/status` expose checkpoint service liveness, dirty counts, recent success/failure, and the last persisted position.
